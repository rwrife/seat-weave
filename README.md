...
**Status: native app with the complete compact-phone seating workflow, private backup and previewed public export, plus the adaptive `SeatingWorkspaceLayout` regular-width workspace, and the issue #6 integrated-reliability evidence layer (deterministic randomized invariants, full-journey store regression test, large-text simulator journey and static offline-privacy audit).** [Bootstrap evidence](docs/bootstrap-evidence.md) records the exact tested commit, toolchain, simulator and results. [Domain evidence](docs/domain-evidence.md) records the issue #2 model, commands, three-state rules and persistence verification. [Workflow evidence](docs/workflow-evidence.md) records the issue #3 compact seating UI, its simulator journey and the honest host limits. [Backup and export evidence](docs/backup-export-evidence.md) records the issue #5 public-export privacy contract, versioned backup/restore validation and confirmed deletion. [Adaptive workspace evidence](docs/adaptive-workspace-evidence.md) records the issue #4 region adapter, selection-continuity tests, the two-destination CI strategy and the accessibility checks still pending human validation. [Integrated reliability evidence](docs/integrated-reliability-evidence.md) records the issue #6 evidence matrix: what CI and the simulator actually prove, and — explicitly unresolved — the physical-iPhone offline journey with observed network capture and the human VoiceOver/keyboard/Switch Control sessions that no CI run can perform. The repository contains a native SwiftUI app, the linked pure Swift domain module with tests, a SwiftData file store and launch, seating-journey, share-journey, workspace-transition and large-text simulator coverage. Simulator success is not physical-device, signing, TestFlight or App Store evidence.
...
- The offline contract is machine-checked: `python3 Scripts/entitlement_audit.py --source-only` (run in CI and documented in [integrated reliability evidence](docs/integrated-reliability-evidence.md)) proves no source file imports networking/CloudKit/tracking frameworks, no networking APIs appear, and the project declares no custom entitlements. This static review is NOT observed network-capture evidence; the on-device fresh-install journey with network observation remains an explicitly open physical-device check.

## Accessibility

VoiceOver labels identify guest, table, seat, assignment and warnings; reading order is logical in both layouts. Support Dynamic Type through accessibility sizes, reduced motion, high contrast, keyboard/Switch Control workflows and 44-point minimum actions. No operation may depend on drag gestures, color, spatial vision or the second pane. List assignment and explicit swap confirmation are first-class UI. The core journey is additionally automated at the extra-extra-large text size on the simulator; human VoiceOver/keyboard/Switch Control sessions on physical hardware remain open and are tracked in the [evidence matrix](docs/integrated-reliability-evidence.md).
