...
**Status: issue #1 native bootstrap implemented; a latest-revision green macOS CI run is pending.** CI run `35000901774` measured the exact pinned Xcode/SDK and passed all four domain tests before exposing a simulator architecture mismatch; commit `a8b270c` corrected that Debug setting. Run `35001629386` selected an explicit simulator but was cancelled at the 45-minute job limit while `simctl boot` hung, before domain/build execution. Simulator boot and readiness are now bounded and failure-provenanced for the next run. The repository contains a native SwiftUI app, a linked pure Swift domain package, baseline tests and a launch smoke test. Event creation and seating are intentionally not implemented yet. No completed simulator app build/test, physical-device, signed binary, TestFlight or App Store release result is claimed. The remaining milestones in [PLAN.md](PLAN.md) define future acceptance.
