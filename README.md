...
**Status: native app with the complete compact-phone seating workflow.** [Bootstrap evidence](docs/bootstrap-evidence.md) records the exact tested commit, toolchain, simulator and results. [Domain evidence](docs/domain-evidence.md) records the issue #2 model, commands, three-state rules and persistence verification. [Workflow evidence](docs/workflow-evidence.md) records the issue #3 compact seating UI, its simulator journey and the honest host limits. The repository contains a native SwiftUI app, the linked pure Swift domain module with tests, a SwiftData event store and launch plus seating-journey simulator coverage. Export and backup flows remain later milestones, not implemented features. Simulator success is not physical-device, signing, TestFlight or App Store evidence.
